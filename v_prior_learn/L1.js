...
function L1(examples, prob = false, stop_early = true) {
    let example_str = JSON.stringify(examples);
    if (solved_instances[example_str] !== undefined){
        return solved_instances[example_str]
    }
    var utters = [];
    // examples to utterances, note here the examples are ordered in order of insertion, despite it being a diciontary, 
    // this is pretty bad practice but stackoverflow says it should be fine
    ...
    for (var j=0; j<subsample_candidates.length; j++){
        ...
        if ((Date.now() - start_time > 2000) && stop_early) {
            break;
        }
    }
    ...
    // console.log("sorted candidates top 3")
    // console.log(sorted_cands.slice(0,10));
    let ret = sorted_cands.map(x => x[1]);
    solved_instances[example_str] = ret;
    return ret;
}

function get_num_syms(shape) {
    var syms = new Set();
    shape.forEach( x => {
        syms.add(JSON.stringify(x[1]));
    });
    return syms.size;
}

function prior_score(shape) {
    var num_sym = get_num_syms(shape);
    return shape.length + num_sym * 0.1;
}

function L_prior(examples, prob = false, stop_early = true) {
    // use learned orderings
    let total_orderings = learned_orderings;
    ...
    let l0_candidates = Array.from(L0(examples));
    // console.log("all l0 sol length ", l0_candidates.length);
    let ordered_candidates = l0_candidates.sort(function(a,b){
        // return prior_shape_order[b] - prior_shape_order[a];
        return total_orderings.indexOf(a) - total_orderings.indexOf(b);
    });
